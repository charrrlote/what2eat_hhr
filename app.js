...
async function fetchRecommendation(name) {
  const response = await fetch('https://api.deepseek.com/v1/chat/completions', {
    ...
    messages: [{
      role: "user",
      content: `我现在很饿，但是不知道吃什么，请你模仿${name}的语气，用搞笑的语气适当的加入符合这个人特质的动词，是任务更生动形象。让他向我推荐菜品，
      菜品需要与段子结合起来，需要包含具体的外面店铺或者菜品名。每次输出格式不固定，尽可能从丰富多样菜品中挑选几个进行推荐，推荐结果中可以有比较少见的菜品，或者各地的特色菜，甚至国外美食。
      尽量保证连续几次推荐的菜品不一致。尽量避免推荐兰州拉面、黄焖鸡和螺蛳粉，多推荐家常菜，若推荐自己做的简单家常菜，可以用${name}的语气教做菜。`
    }],
    temperature: 0.8
  })
  });
